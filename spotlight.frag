...
uniform float LightPenumbraAngle;
...
vec3 spotLight( in vec3 p, in vec3 n, in vec3 v, in vec3 diffuseColor, in vec3 specularColor, in float specularPower)
{
    ...
    float a = cos(LightAngle * DEG2RAD);
    float pa = cos(LightPenumbraAngle * DEG2RAD);
    float ndotl = max(dot(n, l), 0.0);
    float ldotd = dot(-l, normalize(LightDirection));
    vec3 h = normalize(l+v);
    float ndoth = max(dot(n, h), 0.0);
    float fallof = clamp(pow( (ldotd - a) / (a-pa), 4), 0.0, 1.0);
    float d = distance(LightPosition, p);
    float att = 1.f / (d*d);
    return att * fallof * LightColor * LightIntensity * (diffuseColor * ndotl + specularColor * pow(ndoth, specularPower));
}
